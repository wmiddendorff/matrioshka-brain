...
├── tools/
│   └── manifest.md            # Tool documentation (atlas-agent pattern)
├── agents/
│   └── default/
│       ├── sessions/
...
│  (standalone)   │  telegram-queue    │   (skill only)  │
└─────────────────┘                    └─────────────────┘

AFTER (MCP Tool + Skill Hybrid):
┌─────────────────┐                    ┌─────────────────┐
│  Telegram Bot   │ ←──────────────────│   MCP Server    │
│  (standalone)   │    direct calls    │   (tools)       │
└─────────────────┘                    └────────┬────────┘
                                                │
                                       ┌────────▼────────┐
                                       │   Claude Code   │
...
1. **MCP Server** (`~/.mudpuppy/mcp-server/`)
   ```typescript
...
2. **Claude Code Hook** (`~/.claude/hooks.json`)
   ```json
   {
     "user-prompt-submit": [
       {
         "command": "node ~/.mudpuppy/scripts/check-telegram.js",
         "condition": "always"
       }
     ]
   }
   ...
   if (pending.length > 0) {
     console.log(`\n📱 [Telegram] ${pending.length} pending message(s):`);
...
3. **Updated Skill** (streamlined, focuses on persona)
   ...
   ---
   name: mudpuppy
   description: Mudpuppy autonomous companion
   ...
   ## Personality
   Load from ~/.mudpuppy/SOUL.md and ~/.mudpuppy/IDENTITY.md
...
4. **Tool Manifest** (`~/.mudpuppy/tools/manifest.md`)
   ```markdown
   # Mudpuppy Tools
...
- [ ] Write integration tests for tool → bot communication
...
**Memory Tools Manifest** (`~/.mudpuppy/tools/manifest.md`):
```markdown
# Memory Tools
...
The following patterns from the atlas-agent project (`/home/localadmin/Desktop/workspace/git/atlas-agent`) have been incorporated into Mudpuppy:
...
```
score = (vector_score × vector_weight) + (keyword_score × keyword_weight) + bonuses
```
