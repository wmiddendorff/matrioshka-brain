...
- [x] `matrioshka-brain init` creates workspace structure
- [x] `matrioshka-brain config get/set` works
- [x] Config loads from `~/.matrioshka-brain/config.json`
- [x] All paths use `$MATRIOSHKA_BRAIN_HOME` or `~`
...
- [x] Bot starts as daemon via `matrioshka-brain telegram start`
...
- [x] `skills/matrioshka-brain/SKILL.md` - Claude Code skill (persona + 16 MCP tool workflows)
...
- [x] `.gitignore` - Added `.mcp.json`, replaced `.openclaw-clone/` with `.matrioshka-brain/`
...
**Accomplishments:**
- Created `skills/matrioshka-brain/SKILL.md` with YAML frontmatter and all 7 sections (init, personality, messages, memory, heartbeat, soul evolution, security)
- Removed `.mcp.json` from git tracking (`git rm`), added to `.gitignore`
- Created `.mcp.json.example` with placeholder paths
- Updated `.gitignore`: added `.mcp.json`, replaced `.openclaw-clone/` with `.matrioshka-brain/`
...
**Files Created:**
- `skills/matrioshka-brain/SKILL.md`
- `setup.sh`
- `.mcp.json.example`
...
**Files Modified:**
- `.gitignore` - Added .mcp.json, replaced .openclaw-clone/ with .matrioshka-brain/
...
- Removed v1 skill from `~/.claude/skills/matrioshka-brain/`
...
**Workspace:** `~/.matrioshka-brain/` (or `$MATRIOSHKA_BRAIN_HOME`)
**Project:** `~/Desktop/workspace/matrioshka-brain/`
